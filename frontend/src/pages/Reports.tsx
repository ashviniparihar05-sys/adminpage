...
// ✅ AI Case Report generate karta hai
// ✅ "Email Report" button — organizer ka email fetch karo, mailto: se mail app open karo

import React, { useEffect, useState } from 'react';
import {
  Bot, Share2, MoreHorizontal, ExternalLink, Send,
  CheckCircle2, Loader2, RefreshCw, X, Printer,
  AlertTriangle, ShieldAlert, FileText, TrendingUp, User, Calendar, Mail,
} from 'lucide-react';
...
// ─── mailto: link builder ─────────────────────────────────────────────────────
// Mail app mein pre-filled email open karta hai
// Body mein report ka summary dalta hai (mailto: 2000 char limit ke andar)
function buildMailtoLink(
  creatorInfo: CreatorEmailInfo,
  report: Report,
  caseReport: GeneratedCaseReport
): string {
  const subject = encodeURIComponent(
    `[MyApp Moderation] Case ${report.caseId} — ${report.type}: ${report.title}`
  );

  // Short body — mailto: mein full report fit nahi hoti, isliye key points
  const riskScore = caseReport.aiAnalysis?.riskScore ?? 'N/A';
  const riskLevel = (caseReport.aiAnalysis?.riskLevel ?? 'N/A').toUpperCase();
  const flags = caseReport.aiAnalysis?.flags?.map(f => f.type).join(', ') || 'None';

  const bodyLines = [
    `Dear ${creatorInfo.name},`,
    ``,
    `This is an official moderation notice regarding your event on MyApp.`,
    ``,
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    `CASE DETAILS`,
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    `Case ID     : ${report.caseId}`,
    `Event       : ${report.linkedEvent}`,
    `Date        : ${report.linkedEventDate}`,
    `Venue       : ${report.linkedEventVenue}`,
    ...
    ``,
    `Summary:`,
    caseReport.aiAnalysis?.summary ?? 'No summary available.',
    ``,
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    `RECOMMENDED ACTION`,
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    caseReport.recommendedActions[0] ?? 'Please review the case.',
    ``,
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    `CONCLUSION`,
    `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`,
    caseReport.conclusion,
    ``,
    `Report generated on: ${caseReport.generatedAt}`,
    ``,
    `If you have questions, please contact the moderation team.`,
    ``,
    `— MyApp Moderation Team`,
  ];

  const body = encodeURIComponent(bodyLines.join('\n'));
  return `mailto:${creatorInfo.email}?subject=${subject}&body=${body}`;
}
...
const CaseReportModal: React.FC<{
  ...
  // ✅ Creator email state
  const [creatorInfo, setCreatorInfo] = useState<CreatorEmailInfo | null>(null);
  ...
  useEffect(() => {
    ...
    // ✅ Saath mein creator email bhi fetch karo
    fetchCreatorEmail();
  }, [report]);

  // ─── Creator email fetch karo backend se ─────────────────────────────────
  // GET /events/:id/creator-email
  // Server → event se creatorClerkId → users collection → email return karta hai
  const fetchCreatorEmail = async () => {
    setEmailLoading(true);
    setEmailError(null);
    try {
      const res = await fetch(`http://localhost:5006/events/${report.id}/creator-email`);
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || 'Failed to fetch email');
      }
      const data: CreatorEmailInfo = await res.json();
      setCreatorInfo(data);
      console.log(`📧 Creator email loaded: ${data.email}`);
    } catch (err: any) {
      console.warn('Email fetch failed:', err.message);
  ...
  const handleEmailReport = () => {
    if (!creatorInfo || !caseReport) return;
    const mailtoLink = buildMailtoLink(creatorInfo, report, caseReport);
    // mailto: se default mail app khulta hai pre-filled email ke saath
        ...
        <div className="sticky top-0 z-10 bg-white rounded-t-3xl border-b border-slate-100 px-8 py-4 flex items-center justify-between print:hidden">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 
              ...
              1. Modal khulte hi fetchCreatorEmail() call hoti hai
              2. Backend event ka creatorClerkId nikalta hai
              3. Users collection mein us clerkId se email dhundta hai
              4. Email milne ke baad button active ho jaata hai
              5. Click pe mailto: link banta hai — default mail app khulta hai
              6. Mail app mein To, Subject, Body sab pre-filled hota hai
            */}
            {emailLoading ? (
              <div className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 rounded-xl text-xs font-bold text-slate-500">
                <Loader2 size={12} className="animate-spin" />
                Fetching email…
              </div>
            ) : emailError ? (
              <div
                className="flex items-center gap-1.5 px-4 py-2 bg-red-50 rounded-xl text-xs font-bold text-red-500 cursor-pointer hover:bg-red-100 transition-all"
                onClick={fetchCreatorEmail}
                title={`Error: ${emailError} — Click to retry`}
              >
                <Mail size={13} />
                Email N/A
              </div>
              ...
              <button
                ...
                disabled={!caseReport}
                className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
                title={`Send report to ${creatorInfo.email}`}
              >
                <Mail size={13} />
                Email Report
                {/* Small email badge — hover pe full email dikhao */}
                <span className="hidden group-hover:block absolute top-full left-0 mt-1 bg-slate-900 text-white text-[10px] px-2 py-1 rounded-lg whitespace-nowrap z-20">
                  {creatorInfo.email}
                </span>
              </button>
            ) : null}

            {/* Email address display — show karo taaki admin confirm kare */}
            {creatorInfo && (
              <div className="hidden lg:flex items-center gap-1 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl">
                <Mail size={11} className="text-slate-400" />
                <span className="text-[10px] text-slate-500 font-mono">{creatorInfo.email}</span>
              </div>
            )}

            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-4 py-2 bg-slate-100 hover:bg-slate-200 rounded-xl text-xs font-bold text-slate-700 transition-all"
            >
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center bg-slate-100 hover:bg-red-100 rounded-xl text-slate-500 hover:text-red-600 transition-all"
            >
              <X size={16} />
            </button>
          </div>
        ...
        {caseReport && !loading && (
          <div className="p-8 space-y-8 print:p-6">
            ...
            <div className="border-2 border-slate-900 rounded-2xl p-6 print:border-black">
              ...
              {/* ── Organizer Email Banner (report ke andar bhi) ── */}
              {creatorInfo && (
                <div className="mt-4 pt-4 border-t border-slate-200 flex items-center justify-between flex-wrap gap-3">
                  <div className="flex items-center gap-2">
                    <Mail size={14} className="text-slate-500" />
                    <div>
                      <p className="text-[9px] font-bold text-slate-400 uppercase">Organizer</p>
                      <p className="text-xs font-bold text-slate-700">
                        {creatorInfo.name}
                        <span className="font-normal text-slate-500 ml-2 font-mono">{creatorInfo.email}</span>
                      </p>
                    </div>
                  </div>
                  <button
            ...
            {/* Footer — email button bhi yahan */}
            <div className="border-t-2 border-dashed border-slate-200 pt-6 flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-3">
                ...
                {/* Footer mein bhi email button */}
                {creatorInfo && (
                  <button
                    onClick={handleEmailReport}
                    className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-sm print:hidden"
                  >
                    <Mail size={12} />
                    Send to {creatorInfo.name}
                  </button>
                )}
              </div>
            </div>
